...
import { calculatePoliticalScores, getPoliticalScores } from "@/lib/api";
...
import type { PoliticalScoreCalculateResult } from "@/lib/api";
...
import { POLITICAL_SCORE_MARKETING_KEY } from "@/lib/formula-marketing-keys";
...
type ScoreRow = PoliticalScoreCalculateResult["scores"][number];
...
export default function PoliticalScorePage() {
  ...
  const [result, setResult] = useState<PoliticalScoreCalculateResult | null>(null);
  ...
  const loadScores = useCallback(
    async (
      ...
      setRunning(true);
      try {
        let data: PoliticalScoreCalculateResult;
        if (recalculate) {
          const calc = await calculatePoliticalScores(accessToken, body);
          data = await getPoliticalScores(accessToken, body);
          data = {
            ...data,
            scoresWritten: calc.scoresWritten,
            tradesSynced: calc.tradesSynced,
            syncNotes: calc.syncNotes,
            errors: calc.errors,
            tradesUsedInScoring: calc.tradesUsedInScoring,
          };
        } else {
          data = await getPoliticalScores(accessToken, body);
        }
          ...
          if (recalculate) {
            showSuccess(
              data.scoresWritten > 0
                ? `Updated ${data.scoresWritten} scores (${data.tickersWithData} tickers; ${data.tradesSynced} trade rows synced)`
                : "Run finished; no scores written. Check errors below."
            );
          } else if (data.tickersWithData > 0) {
            showSuccess(`Loaded ${data.tickersWithData} political scores`);
          }
        }
      } catch (err) {
    ...
    <div className="min-h-0 px-4 py-8 sm:px-8 md:px-10 lg:px-12">
      <div className="mx-auto max-w-6xl space-y-8">
          ...
          <div className="min-w-0 space-y-2">
            ...
            {showSubheading ? (
              <p className="text-base text-muted-foreground">
                Congressional trade signal (−100 to +100) from disclosures, committees, and
                clustering. Scores load automatically; use custom run to filter by tickers,
                limit, or score range.
              </p>
            ) : null}
          </div>
        ...
        <FormulaPageResultsSettingsTabs
          results={
            <>
        <FormulaExplanationSection formulaKey={POLITICAL_SCORE_MARKETING_KEY} />
        ...
        <section className="rounded-2xl border border-border bg-card p-4 space-y-3 shadow-sm">
          ...
          <p className="text-xs text-muted-foreground">
            Applied on the next run (inclusive, −100–100). Leave empty for no bound.
          </p>
        ...
        <Dialog open={modalOpen} onOpenChange={setModalOpen}>
          <DialogContent className="sm:max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription>
                Optional max securities, ticker list, and score bounds. Leave empty for
                defaults.
              </DialogDescription>
            </DialogHeader>
            <form id="ps-modal-form" onSubmit={onModalSubmit} className="space-y-4">
              <div className="space-y-2">
                <FormLabel
                  htmlFor="ps-modal-limit"
                  className="text-xs font-medium text-muted-foreground"
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2">
                  <FormLabel htmlFor="ps-modal-min">Min score</FormLabel>
                  <FormInput
                    id="ps-modal-min"
                    inputMode="decimal"
                    value={minScore}
                ...
                <div className="space-y-2">
                  <FormLabel htmlFor="ps-modal-max">Max score</FormLabel>
                  <FormInput
                    id="ps-modal-max"
                    inputMode="decimal"
                    value={maxScore}
              ...
              <p className="text-xs leading-relaxed text-muted-foreground">
                Weights: 35% committee relevance, 20% trade size, 20% recency, 15%
                influence, 10% cluster; then buy vs sell pressure (Formulas.md).
              </p>
            </form>
            <DialogFooter>
              ...
              <PrimaryButton type="submit" form="ps-modal-form" disabled={running || !accessToken}>
        ...
        {result ? (
          <section className="space-y-4">
            ...
            <ul className="text-sm text-muted-foreground space-y-1">
              ...
              <li>Trade disclosures synced (rows): {result.tradesSynced}</li>
              <li>
                Trades used in scoring (180d window):{" "}
                {result.tradesUsedInScoring ?? 0}
              </li>
            </ul>
                            ...
                            <TableRow>
                              <TableCell colSpan={4} className="bg-muted/30">
                                <ScoreExplanationDetails
                                  formulaKey={POLITICAL_SCORE_MARKETING_KEY}
                                  explanation={{
            ...
            {(result.syncNotes?.length ?? 0) > 0 ? (
              <div className="rounded-md border border-border/80 bg-muted/30 p-4">
                <p className="text-sm font-medium text-foreground mb-2">
                  FMP sync (informational)
                </p>
                <ul className="text-xs text-muted-foreground space-y-1 max-h-96 overflow-y-auto">
                  {(result.syncNotes ?? []).map((n, i) => (
                    <li key={`sync-${i}`}>
                      <span className="font-mono text-foreground/80">{n.ticker}</span>:{" "}
                      {n.message}
                    </li>
                  ))}
                </ul>
